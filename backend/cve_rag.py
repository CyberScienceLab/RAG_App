...
def parse_cve_numbers_from_code(cve_string):
    pattern = r'CVE-(\d{4})-(\d{4,7})'
...
def format_second_number_set(second_set):
    if len(second_set) == 4:
...
def retrieve_cve_descriptions(cves: list[str]) -> tuple[str, list[str]]:  
    cve_descriptions = ''
    not_found = []

    for cve in cves:
        try:
            first_set, second_set = parse_cve_numbers_from_code(cve)
            second_number_formatted = format_second_number_set(second_set)
...
def build_messages(prompt: str, extra_context: str, cve_descriptions: str) -> list[str, str]:
    return [
        {
            'role': 'system',
            'content': f'''You are an advanced CVE information system designed to assist users in verifying the usage of CVEs (Common Vulnerabilities and Exposures) in security reports and providing detailed descriptions of CVEs. Your responsibilities include:
                            ...
                            1. Verifying CVE Usage: Analyze text from security reports and determine if it references any CVEs. If CVEs are mentioned:
                            ...
                            Format Requirements:
                            - For CVE descriptions, include the CVE ID, Vendor, Product and description.
                            - For verifying usage, highlight any CVE references found in the report and provide relevant details.

                            Examples:
                            1. Input: "Provide information about the following CVEs CVE-2021-22986 and CVE-2022-22987."
                            Output: 
                            - CVE-2021-22986: [ID, Vendor, Product, Description]
                            - CVE-2022-22987: [ID, Vendor, Product, Description]

                            2. Input: "Please verify the CVE usage in this report?"
                            ...
                            File Passed / Extra context for the prompt: 
                            {extra_context}
                        '''
        },
        {
...
def get_messages_with_context(prompt: str, extra_context: str, num_chunks: int) -> list[str, str]:
    all_text = prompt + extra_context

    all_cves = extract_cves_from_text(all_text)

    cve_descriptions, cves_not_found = retrieve_cve_descriptions(all_cves)

    if len(cves_not_found) > 0:
        # work with aeiyan on this part since he made llamasug
        pass

    return build_messages(prompt, extra_context, cve_descriptions)
